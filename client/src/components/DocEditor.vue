...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      //   fetch('/public/docs/' + this.id + '.md').then(res=>res.text()).then(res=>this.content=res)
    ...
    methods: {
      ...
      save() {
        const { title, author, content, tag } = this
        this.$emit('save', { title, author, content, tags: tag.split(' ') })
      }
    }
  }
</script>
